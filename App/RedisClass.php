...
class RedisClass
{
    ...
    public static function getInstance($config, $attr = array())
    {
        ...
        $k    =    md5(implode('_', $config).$attr['db_id']);
        if(!isset(static::$_instance[$k]) or ! (static::$_instance[$k] instanceof self))
        {
     ...
     * 在队列头部插入多个个元素
     * @param unknown $key
     * @param unknown $list array
     * 返回队列长度
     */
    public function lPushList( $key , $list )
    {
     ...
     * @param unknown $key
     */
    public function lPopList($key,$num=1)
    {
        $this->redis->multi(Redis::PIPELINE);
        $this->redis->lRange($key,0,$num-1);
        $this->redis->ltrim($key,$num,-1);
        $res = $this->redis->exec();
        if($res && isset($res[0])) return $res[0];
    }
     ...
     * @param unknown $key
     */
    public function rPopList($key,$num=1)
    {
        $this->redis->multi(Redis::PIPELINE);
        $this->redis->lRange($key,-$num,-1);
